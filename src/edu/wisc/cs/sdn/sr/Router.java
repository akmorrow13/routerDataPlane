...
public class Router 
{
	...
	private void reRouteNonInterface(Ethernet etherPacket, Iface inIface) {
		...
		if(ttl <= 0) {
			...
			sendICMPMessage(inIface.getIpAddress(), ipPacket.getSourceAddress(), (byte) 0, (byte) 11, ipClone);
			return;
		...
		if(rtEntry != null) { // If one route was found in the route table.
			...
			if(rtEntry.getGatewayAddress() == 0) { 
				...
				// Check if the path is available. If the cost is greater than 16, than it is not
				// a valid path. Send an error.
